package com.zcj.blg.phoneanswer.database;
...
public class DataBaseManager {
    ...
    public List<AnSwerInfo> getAnSwers(int mode, String number, String testNo) {
        ...
        switch (mode) {//questionType='1'and
            case 0:
                sql = "select questionId,questionName,optionA,optionB,optionC,optionD,optionE,correctAnswer,questionType,answerAnalysis,video_url from TestData where test_No='" + testNo + "' order by questionType desc";
                break;
            case 1:
                sql = "select questionId,questionName,optionA,optionB,optionC,optionD,optionE,correctAnswer,questionType,answerAnalysis,video_url FROM TestData  where  video_url='' ORDER BY RANDOM() limit " + number;
                break;
            case 2:
                sql = "select questionId,questionName,optionA,optionB,optionC,optionD,optionE,correctAnswer,questionType,answerAnalysis,video_url from TestData where errornumber>0";
                break;
            default:
                sql = null;
        }
